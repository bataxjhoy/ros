...
void turn_on_robot::Cmd_Vel_Callback(const geometry_msgs::Twist &twist_aux)
{
  ...
  //机器人x轴的目标线速度
  transition=0;
  transition = twist_aux.linear.x * 1000 * scale_factor_x; //将浮点数放大一千倍，简化传输
  ...
  //机器人z轴的目标角速度
  transition=0;
  transition = twist_aux.angular.z * 1000 * scale_factor_x;
  Send_Data.tx[8] = transition;
  Send_Data.tx[7] = transition>>8;
...
//下发给采集板
void turn_on_robot::Cmd_Vel_Callback_Ex(const geometry_msgs::Twist &twist_aux)
{
  short  transition;  //intermediate variable //中间变量
  // ROS_WARN("Cmd_Vel_Callback_Ex [x: %.3f, y: %.3f, a: %.3f]", twist_aux.linear.x, twist_aux.linear.y, twist_aux.angular.z);

  Send_Data.tx[0] = FRAME_HEADER; //frame head 0x7B //帧头0X7B
  Send_Data.tx[1] = 0x0; //set aside //功能码
  Send_Data.tx[2] = 0x01; //set aside //车端

  Send_Data.tx[3] = 0x09; //Data length

  //The target velocity of the X-axis of the robot
  //机器人x轴的目标线速度
  transition=0;
  transition = twist_aux.linear.x * 1000 * scale_factor_x; //将浮点数放大一千倍，简化传输
  Send_Data.tx[5] = transition;     //取数据的低8位
  ...
  //机器人z轴的目标角速度
  transition=0;
  transition = twist_aux.angular.z * 1000 * scale_factor_z;
  Send_Data.tx[9] = transition;
  Send_Data.tx[8] = transition>>8;

  Send_Data.tx[10] = robot_status;  //robot status

  Send_Data.tx[11] = 0x00;          //robot error code
  Send_Data.tx[12] = 0x00;          //water error code

  Send_Data.tx[13]=Check_Sum(12, SEND_DATA_CHECK); //For the BCC check bits, see the Check_Sum function //BCC校验位，规则参见Check_Sum函数
  Send_Data.tx[14]=FRAME_TAIL; //frame tail 0x7D //帧尾0X7D

  // for (int i = 0; i < 15; i++) {
  //   printf("%02x ", Send_Data.tx[i]);
  // }
  // printf("\n");
...
void turn_on_robot::syncRobotStatus(const std_msgs::UInt32::ConstPtr& msg)
{
  int transition = 0; 
  Send_Data.tx[0] = FRAME_HEADER; //frame head 0x7B //帧头0X7B
  Send_Data.tx[1] = 0x0; //set aside //功能码
  ...
  Send_Data.tx[10] = msg->data;  //robot status
...
void turn_on_robot::getSetScaleXCallback(const std_msgs::Float32::ConstPtr& msg)
{
	float scale = (msg->data);
	ROS_ERROR("getSetScaleXCallback x scale: %.3f", scale);
	scale_factor_x = scale;
}

void turn_on_robot::getSetScaleZCallback(const std_msgs::Float32::ConstPtr& msg)
{
	float scale = (msg->data);
	ROS_ERROR("getSetScaleZCallback z scale: %.3f", scale);
	scale_factor_z = scale;
}

void turn_on_robot::getSyncEmbDataCmdCallback(const std_msgs::UInt8::ConstPtr& msg)
{
	int sync = (msg->data);
	if (!sync)
		update_flag = false;
...
void turn_on_robot::getClearCacheDataCmdCallback(const std_msgs::String::ConstPtr & msg)
{
	memset(&Robot_Pos, 0, sizeof(Robot_Pos));
  	memset(&Robot_Vel, 0, sizeof(Robot_Vel));
	Publish_Odom();
	ROS_ERROR("clean cache data finish!");
}
...
void turn_on_robot::Publish_ImuSensor()
{
  ...
  // if(Robot_Vel.Z != 0.0f){
  //   ROS_WARN("gyro_w= %f,cur_w= %f,cur_v= %f", Mpu6050.angular_velocity.z, Robot_Vel.Z, Robot_Vel.X);
  // }
...
void turn_on_robot::Publish_Odom()
{
    ...
    // ROS_ERROR("publish odom pose [%f, %f, %f], twist [%f, %f, %f]", 
...
/**************************************
Date: January 28, 2021
Function: Publish di information
功能: 发布di相关信息
***************************************/
void turn_on_robot::Publish_di()
{
    std_msgs::UInt8 di_msgs; //Define the data type of the power supply voltage publishing topic //定义电源电压发布话题的数据类型
    
    di_msgs.data = di_status; 
    di_publisher.publish(di_msgs); 
}
...
功能: 发布di相关信息
***************************************/
void turn_on_robot::Publish_ir()
{
	std_msgs::UInt32 ir_msgs; 

	static int count_status_pub = 0;
    if(count_status_pub++ > 10) {
		count_status_pub = 0;  
    	ir_msgs.data = ir_status; 
    	ir_publisher.publish(ir_msgs); 
    }
}

/**************************************
Date: January 28, 2021
Function: Publish status information
功能: 发布status相关信息
***************************************/
void turn_on_robot::Publish_status()
{
    std_msgs::UInt8 status_msgs; //Define the data type of the power supply voltage publishing topic //定义电源电压发布话题的数据类型
    static int count_status_pub = 0;
    if(count_status_pub++ > 10) {
      count_status_pub = 0;  
      status_msgs.data = robot_status; //The power supply voltage is obtained //电源供电的电压获取
      status_publisher.publish(status_msgs); //Post the power supply voltage topic unit: V, volt //发布电源电压话题单位：V、伏特
    }
}

void turn_on_robot::Publish_register()
{
    std_msgs::String msg;
    std::stringstream msgStr;
    msgStr << "robot";
    msg.data = msgStr.str();
    register_publisher.publish(msg);
    ROS_ERROR("register robot module to common node");
}

void turn_on_robot::Publish_src_x_vel()
{
    std_msgs::Float32 src_x; 

    src_x.data = Robot_Vel.X; 
    src_x_vel_publilsher.publish(src_x);
}

void turn_on_robot::Publish_src_z_vel()
{
    std_msgs::Float32 src_z; 

    src_z.data = Robot_Vel.Z; 
    src_z_vel_publilsher.publish(src_z);
}

/**************************************
Date: January 28, 2021
Function: Publish ultrasound information
功能: 发布ultrasound相关信息
***************************************/
void turn_on_robot::Publish_ultrasound()
{
    // std_msgs::Float32 voltage_msgs; //Define the data type of the power supply voltage publishing topic //定义电源电压发布话题的数据类型
...
bool turn_on_robot::Get_Sensor_Data_Ex()
{ 
  short transition_16=0, j=0, Header_Pos=0, Tail_Pos=0, date_len = 0, totoal_len=0; //Intermediate variable //中间变量
  uint8_t Receive_Head_Pr[4] = {0};
  uint8_t Receive_Data_Pr[RECEIVE_DATA_SIZE] = {0}; //Temporary variable to save the data of the lower machine //临时变量，保存下位机数据
  uint8_t Receive_Total_Pr[RECEIVE_DATA_SIZE] = {0};
  
  while( ! Stm32_Serial.waitReadable () );//等待可读
  
  Stm32_Serial.read(Receive_Data_Pr,sizeof (Receive_Data_Pr));
  
  //打印显示
  // for (int i = 0; i < sizeof(Receive_Data_Pr); i++) {
  //       ROS_INFO("%02x ", Receive_Data_Pr[i]);
  //   }  
  // ROS_INFO("---------------------------------");//分界线

  for(j=0;j<RECEIVE_DATA_SIZE;j++)
  {
    if(Receive_Data_Pr[j]==FRAME_HEADER)//帧头位置
    Header_Pos=j;
    else if(Receive_Data_Pr[j]==FRAME_TAIL)//帧尾位置
    Tail_Pos=j;    
  }
  if(Tail_Pos==(Header_Pos+RECEIVE_DATA_SIZE-1))//数据包没有交错
  {  
    memcpy(Receive_Data.rx, Receive_Data_Pr, sizeof(Receive_Data_Pr));
  }
  else if(Header_Pos==(1+Tail_Pos))//数据包交错
  {   
    for(j=0;j<RECEIVE_DATA_SIZE;j++)
    Receive_Data.rx[j]=Receive_Data_Pr[(j+Header_Pos)%RECEIVE_DATA_SIZE];//调换位置
  }
  else 
  {      
    return false;
  ...
  if (Receive_Data.Frame_Header == FRAME_HEADER ) //Judge the frame header //判断帧头
  {
    if (Receive_Data.Frame_Tail == FRAME_TAIL) //Judge the end of the frame //判断帧尾
    { 
      int checkSum = Check_Sum(RECEIVE_DATA_SIZE - 2, READ_DATA_CHECK);
      if (Receive_Data.rx[RECEIVE_DATA_SIZE - 2] == checkSum||(Header_Pos==(1+Tail_Pos))) //BCC check passes or two packets are interlaced //BCC校验通过或者两组数据包交错
      {
        Receive_Data.Flag_Stop=Receive_Data.rx[1]; //set aside //预留位
        if (0 == Receive_Data.Flag_Stop) {
          //6 bytes
          Robot_Vel.X = Odom_Trans(Receive_Data.rx[4],Receive_Data.rx[5]); //Get the speed of the moving chassis in the X direction //获取运动底盘X方向速度
          Robot_Vel.Y = Odom_Trans(Receive_Data.rx[6],Receive_Data.rx[7]); //Get the speed of the moving chassis in the Y direction, The Y speed is only valid in the omnidirectional mobile robot chassis//获取运动底盘Y方向速度，Y速度仅在全向移动机器人底盘有效
          Robot_Vel.Z = Odom_Trans(Receive_Data.rx[8],Receive_Data.rx[9]); //Get the speed of the moving chassis in the Z direction //获取运动底盘Z方向速度   
          
          //Get the battery voltage Receive_Data.rx[10],Receive_Data.rx[11] -> wPowerVoltage / mV
          //获取电池电压 2bytes 
          transition_16 = 0;
          transition_16 |=  Receive_Data.rx[10] << 8;
          transition_16 |=  Receive_Data.rx[11];  
          Power_voltage = transition_16/1000+(transition_16 % 1000)*0.001; //Unit conversion millivolt(mv)->volt(v) //单位转换毫伏(mv)->伏(v)

          //MPU6050 stands for IMU only and does not refer to a specific model. It can be either MPU6050 or MPU9250
          //Mpu6050仅代表IMU，不指代特定型号，既可以是MPU6050也可以是MPU9250 12bytes
          Mpu6050_Data.accele_x_data = IMU_Trans(Receive_Data.rx[12],Receive_Data.rx[13]);   //Get the X-axis acceleration of the IMU     //获取IMU的X轴加速度  
          Mpu6050_Data.accele_y_data = IMU_Trans(Receive_Data.rx[14],Receive_Data.rx[15]); //Get the Y-axis acceleration of the IMU     //获取IMU的Y轴加速度
          Mpu6050_Data.accele_z_data = IMU_Trans(Receive_Data.rx[16],Receive_Data.rx[17]); //Get the Z-axis acceleration of the IMU     //获取IMU的Z轴加速度
          Mpu6050_Data.gyros_x_data = IMU_Trans(Receive_Data.rx[18],Receive_Data.rx[19]);  //Get the X-axis angular velocity of the IMU //获取IMU的X轴角速度  
          Mpu6050_Data.gyros_y_data = IMU_Trans(Receive_Data.rx[20],Receive_Data.rx[21]);  //Get the Y-axis angular velocity of the IMU //获取IMU的Y轴角速度  
          Mpu6050_Data.gyros_z_data = IMU_Trans(Receive_Data.rx[22],Receive_Data.rx[23]);  //Get the Z-axis angular velocity of the IMU //获取IMU的Z轴角速度  
          ...
          Mpu6050.angular_velocity.y =  Mpu6050_Data.gyros_y_data * GYROSCOPE_RATIO;
          Mpu6050.angular_velocity.z =  Mpu6050_Data.gyros_z_data * GYROSCOPE_RATIO;
          // ROS_ERROR("imu src line [%f, %f, %f], angle [%f, %f, %f]", 
          //   Mpu6050.linear_acceleration.x, Mpu6050.linear_acceleration.y, Mpu6050.linear_acceleration.z,
          //   Mpu6050.angular_velocity.x, Mpu6050.angular_velocity.y, Mpu6050.angular_velocity.z);

          //TODO DI byDiStatus = Receive_Data.rx[24]; 1bytes contains ir status
          di_status = Receive_Data.rx[24];
          // ROS_ERROR("di status [0x%02x]", di_status); 

          //TODO Ultrasound  Receive_Data.rx[25] Receive_Data.rx[26] Receive_Data.rx[27] Receive_Data.rx[28]  4bytes
          // wheelA = Odom_Trans(Receive_Data.rx[25], Receive_Data.rx[26]);
          // wheelB = Odom_Trans(Receive_Data.rx[27], Receive_Data.rx[28]);
          // wheelA1 = Odom_Trans(Receive_Data.rx[29], Receive_Data.rx[30]);
          // wheelB1 = Odom_Trans(Receive_Data.rx[31], Receive_Data.rx[32]);
          // U1 replace with ir_status
          ir_status = (Receive_Data.rx[25] << 8) | Receive_Data.rx[26];

          // if ((wheelA != 0.0) || (wheelB != 0.0) || (wheelA1 != 0.0) || (wheelB1 != 0.0))
          //   ROS_ERROR("wheelA = %f, wheelB = %f ,wheelA1 = %f, wheelB1 = %f, do = 0x%02x:", wheelA, wheelB, wheelA1, wheelB1, Receive_Data.rx[33]);
          
          //TODO robot status Receive_Data.rx[29] 1byte
          robot_status = Receive_Data.rx[29];

            //TODO water status Receive_Data.rx[30] 1byte

            //TODO clean water flow Receive_Data.rx[31] 1byte

            //TODO sewage water flow Receive_Data.rx[32] 1byte

            //TODO water di status Receive_Data.rx[33] 1byte

          //TODO error code Receive_Data.rx[30] Receive_Data.rx[31] 2bytes

        } else if (2 == Receive_Data.Flag_Stop) { 

        } else if (3 == Receive_Data.Flag_Stop) {

        } else {
          ROS_WARN("undefined cmd type, %d", Receive_Data.Flag_Stop);
        }

        return true;
     } else {
        ROS_WARN("check sum error, src[%02x] -- calc[%02x]", Receive_Data.rx[totoal_len - 2], checkSum);
     }
    }
  } 
  return false;
...
void turn_on_robot::Control()
{
  ...
  while(ros::ok())
  {
    ...
    if (true == Get_Sensor_Data_Ex()) //The serial port reads and verifies the data sent by the lower computer, and then the data is converted to international units
...
turn_on_robot::turn_on_robot():Sampling_Time(0),Power_voltage(0)
{
  ...
  private_nh.param<std::string>("akm_cmd_vel",      akm_cmd_vel,      "akm_cmd_vel"); //Velocity control command //速度控制指令
  ...
  // private_nh.param("scale_factor", scale_factor, 1.0);
  private_nh.param<float>("scale_factor_x", scale_factor_x, 1.0);//距离标定系数
  private_nh.param<float>("scale_factor_z", scale_factor_z, 1.0);//旋转标定系数
  ...
  di_publisher = n.advertise<std_msgs::UInt8>("di_status", 10);
  ir_publisher = n.advertise<std_msgs::UInt32>("ir_status", 10); 
  status_publisher = n.advertise<std_msgs::UInt8>("robot_status", 10);
  register_publisher = n.advertise<std_msgs::String>("/dm_register", 1);
  src_x_vel_publilsher = n.advertise<std_msgs::Float32>("/dm_src_x", 10);
  src_z_vel_publilsher = n.advertise<std_msgs::Float32>("/dm_src_z", 10);
  ultrasound_publisher = n.advertise<std_msgs::Float32>("ultrasound", 10);

  //Set the velocity control command callback function
  //速度控制命令订阅回调函数设置
  Cmd_Vel_Sub     = n.subscribe(cmd_vel,     100, &turn_on_robot::Cmd_Vel_Callback_Ex, this); 
  ...
  status_sub = n.subscribe("dm_ros_status", 1, &turn_on_robot::syncRobotStatus, this);
  scale_x_sub = n.subscribe("set_scale_x", 1, &turn_on_robot::getSetScaleXCallback, this);
  scale_z_sub = n.subscribe("set_scale_z", 1, &turn_on_robot::getSetScaleZCallback, this);
  sync_emb_data = n.subscribe("dm_sync_data", 1, &turn_on_robot::getSyncEmbDataCmdCallback, this);
  clear_cache_sub = n.subscribe("dm_clear_cache", 1, &turn_on_robot::getClearCacheDataCmdCallback, this);
  ROS_ERROR("scale_factor = [%.03f, %.3f]", scale_factor_x, scale_factor_z);
...
turn_on_robot::~turn_on_robot()
{
  ...
  Send_Data.tx[0] = FRAME_HEADER; //frame head 0x7B //帧头0X7B
  Send_Data.tx[1] = 0; //set aside //预留位
  Send_Data.tx[2] = 0x01; //set aside //预留位

  Send_Data.tx[3] = 0x09; //Data length

  //The target velocity of the X-axis of the robot //机器人x轴的目标线速度
  Send_Data.tx[5] = 0;     //取数据的低8位
  Send_Data.tx[4] = 0;  //取数据的高8位

  //The target velocity of the Y-axis of the robot //机器人y轴的目标线速度
  Send_Data.tx[7] = 0;
  Send_Data.tx[6] = 0;

  //The target angular velocity of the robot's Z axis //机器人z轴的目标角速度
  Send_Data.tx[9] = 0;
  Send_Data.tx[8] = 0;

  Send_Data.tx[10] = 0;  //robot status

  Send_Data.tx[11] = 0x00;          //robot error code
  Send_Data.tx[12] = 0x00;          //water error code

  Send_Data.tx[13]=Check_Sum(12, SEND_DATA_CHECK); //For the BCC check bits, see the Check_Sum function //BCC校验位，规则参见Check_Sum函数
  Send_Data.tx[14]=FRAME_TAIL;
  #endif
  try
  {

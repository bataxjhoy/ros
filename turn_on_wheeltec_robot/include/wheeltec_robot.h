...
class turn_on_robot
{
	...
	private:
		...
		ros::Subscriber Akm_cmd_Vel_Sub; //Initialize the topic subscriber //初始化话题订阅者
		ros::Subscriber status_sub;
		ros::Subscriber scale_x_sub;
		ros::Subscriber scale_z_sub;
		ros::Subscriber sync_emb_data;
		ros::Subscriber clear_cache_sub;
		...
		void Akm_cmd_Vel_Callback(const ackermann_msgs::AckermannDriveStamped &akm_ctl); 
		...
		void Cmd_Vel_Callback_Ex(const geometry_msgs::Twist &twist_aux);  

		void syncRobotStatus(const std_msgs::UInt32::ConstPtr& msg);
		void getSetScaleXCallback(const std_msgs::Float32::ConstPtr& msg);
		void getSetScaleZCallback(const std_msgs::Float32::ConstPtr& msg);   
		void getSyncEmbDataCmdCallback(const std_msgs::UInt8::ConstPtr& msg);
		void getClearCacheDataCmdCallback(const std_msgs::String::ConstPtr& msg);

		ros::Publisher odom_publisher, imu_publisher, voltage_publisher; //Initialize the topic publisher //初始化话题发布者

		ros::Publisher di_publisher;
		ros::Publisher ir_publisher;
		ros::Publisher status_publisher;
		ros::Publisher register_publisher;
		ros::Publisher src_x_vel_publilsher;
		ros::Publisher src_z_vel_publilsher;
		ros::Publisher ultrasound_publisher;
		...
		void Publish_ir(); 
		void Publish_status();
		void Publish_src_x_vel();
		void Publish_src_z_vel();
        ...
        string usart_port_name, robot_frame_id, gyro_frame_id, odom_frame_id, cmd_vel, akm_cmd_vel; //Define the related variables //定义相关变量
        float scale_factor_x, scale_factor_z;
        ...
        float wheelA, wheelB, wheelA1, wheelB1;
        int di_status;
		int ir_status;
        int robot_status;
		bool update_flag;

...
// Pose hypothesis 位置假设
typedef struct
{
  // Total weight (weights sum to 1) 总的权重
  double weight;

  // Mean of pose esimate 平均位置估计
  pf_vector_t pf_pose_mean;

  // Covariance of pose estimate 位置协方差估计
  pf_matrix_t pf_pose_cov;

} amcl_hyp_t;

static double normalize(double z) //归一化
{
  return atan2(sin(z),cos(z));
}
static double angle_diff(double a, double b) //角度差
{
  double d1, d2;
  a = normalize(a);
  b = normalize(b);
 ...
 *这个函数只在整个代码使用带有斜杠的旧的rosbags时有用  
 */
inline std::string stripSlash(const std::string& in) //去除条斜线
{
  std::string out = in;
  if ( ( !in.empty() ) && (in[0] == '/') )
    out.erase(0,1);
...
std::vector<std::pair<int,int> > AmclNode::free_space_indices;//自由空间指标
...
{
  // Save latest pose as we're shutting down. 保存最近的位置当关机时
  ROS_WARN("signal sigintHandler to save pose");
...
int main(int argc, char** argv)
{
  ...
  // Make our node available to sigintHandler 复位节点
        ...
        first_reconfigure_call_(true)
{
  ...
  private_nh_.param("use_map_topic", use_map_topic_, false); //是否使用map_topic
  private_nh_.param("first_map_only", first_map_only_, false);

  double tmp;
  private_nh_.param("gui_publish_rate", tmp, -1.0); //发布频率
  gui_publish_period = ros::Duration(1.0/tmp); //周期
  private_nh_.param("save_pose_rate", tmp, 0.5);//保存频率
  save_pose_period = ros::Duration(1.0/tmp); //周期

  private_nh_.param("laser_min_range", laser_min_range_, -1.0);//激光最小范围
  private_nh_.param("laser_max_range", laser_max_range_, -1.0);//
  private_nh_.param("laser_max_beams", max_beams_, 30);//激光光束
  private_nh_.param("min_particles", min_particles_, 100);//最新粒子
...
void AmclNode::reconfigureCB(AMCLConfig &config, uint32_t level)
{
  ...
  ROS_ERROR("reconfigureCB pf_init [%d]", __LINE__);
...
void AmclNode::savePoseToServer()
{
  ...
  //TODO save pose date to file
  std::ofstream storeFile("/home/ximei/robot_robot/cfg/pose.txt");
  if (!storeFile) {
      ROS_WARN("can not open store pose file!");
  } else {
      storeFile << map_pose.getOrigin().x() << " " 
      ...
      //   last_published_pose.pose.covariance[6*0+0], last_published_pose.pose.covariance[6*1+1], last_published_pose.pose.covariance[6*5+5]);
  }
}

void AmclNode::updatePoseFromServer()
{
  double storePose[6] = {0};
  // TODO get pose data fron file
  std::ifstream storeFile("/home/ximei/robot_robot/cfg/pose.txt");
  if (!storeFile) {
    ROS_WARN("can not open store pose file!");
    memset(storePose, 0x00, sizeof(storePose));
  } else {
    int i = 0;
    double tmp = 0.0;
    while(storeFile >> tmp) {
      ROS_WARN("get store pose param: %.3f", tmp);
      storePose[i++] = tmp;
    }
    if ((0 == storePose[3]) && (0 == storePose[4]) && (0 == storePose[5])) {
      storePose[3] = 0.25;
      storePose[4] = 0.25;
      storePose[5] = 0.068539;
    }
    ROS_INFO("open store pose file success!");
    storeFile.close();
  }

  init_pose_[0] = storePose[0];
  init_pose_[1] = storePose[1];
  init_pose_[2] = storePose[2];
  init_cov_[0] = 0.5 * 0.5; //storePose[3];//
  init_cov_[1] = 0.5 * 0.5; //storePose[4];//
  init_cov_[2] = (M_PI/12.0) * (M_PI/12.0); //storePose[5];//
...
AmclNode::handleMapMessage(const nav_msgs::OccupancyGrid& msg)
{
  ...
  ROS_ERROR("in handleMapMessage pf_init [%d]", __LINE__);
  pf_init(pf_, pf_init_pose_mean, pf_init_pose_cov);
  pf_init_ = false;
  ...
  ROS_ERROR("handleMapMessage call applyInitialPose[%d]", __LINE__);
  applyInitialPose();
                      ...
                      const ros::Time& t, const std::string& f)
{
  ...
  // ROS_WARN("getOdomPose [x: %.03f, y: %.03f, a: %.03f]", x, y, yaw);
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  if(frame_to_laser_.find(laser_scan_frame_id) == frame_to_laser_.end())
  {
    ...
    ROS_WARN("Received laser's pose wrt robot: %.3f %.3f %.3f",
              laser_pose_v.v[0],
              laser_pose_v.v[1],
              laser_pose_v.v[2]);
  ...
  //--- laserPose: %.3f %.3f %.3f  laser_pose_v.V[0], laser_pose_v.V[1], laser_pose_v.V[2],
  // ROS_WARN("getOdomPose: %.3f %.3f %.3f  --- tmpPose: %.3f %.3f %.3f",
  ...
  if(pf_init_)
  {
    ...
    // ROS_WARN("Compute change in pose pf_init_ = %d", pf_init_);
  ...
  if(!pf_init_)
  {
    ROS_WARN("Pose at last filter update pf_init_ = %d ", pf_init_);
...
//    ROS_WARN("The robot has moved, update the filter update pf_init_ = %d", pf_init_);
...
//    ROS_ERROR("hyps size: %d", pf_->sets[pf_->current_set].cluster_count);
    for(int hyp_count = 0;
        hyp_count < pf_->sets[pf_->current_set].cluster_count; hyp_count++)
    {
      ...
      //only for test 
      pf_sample_set_t *set = pf_->sets + pf_->current_set;
      pf_cluster_t *cluster = set->clusters + hyp_count;
//      ROS_ERROR("hyp_count = %d, set->cluster_count = %d, pose: %.3f %.3f %.3f", 
//        hyp_count, set->cluster_count, cluster->mean.v[0], cluster->mean.v[1], cluster->mean.v[2]);
...
//               hyps[max_weight_hyp].pf_pose_mean.v[2]);
  ...
  else if(latest_tf_valid_)
  {
    ...
    {
      // ROS_WARN("Is it time to save our last pose to the param server");
...
AmclNode::handleInitialPoseMessage(const geometry_msgs::PoseWithCovarianceStamped& msg)
{
  ...
  ROS_ERROR("handleInitialPoseMessage call applyInitialPose[%d]", __LINE__);
  applyInitialPose();
}
...
AmclNode::applyInitialPose()
{
  ...
  if( initial_pose_hyp_ != NULL && map_ != NULL ) {
    ROS_ERROR("in applyInitialPose pf_init[%d]", __LINE__);
